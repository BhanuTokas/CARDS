"""Phrasing-ensemble ablation for the CUB masking hybrid, prompted
directly ("Can we also try prompt variations and ensembling"). Identical
to run_cards_cub_masking_hybrid_best_of_family_full.py in every respect
EXCEPT the query is an average of the baseline phrasing (PREFIX_
TEMPLATES) and a second, differently-structured phrasing
(ALT_PREFIX_TEMPLATES, already built for the plain-CARDS ablation in
ablate_cards_cub_attributes.py -- reused directly, not reinvented).
demean_query=True kept (matching v67's baseline), orthogonalize left
off -- isolates the phrasing effect alone, same one-factor-at-a-time
discipline as v70/v71.
"""
...
def build_ensemble_query(prefix: str, value: str, encoder) -> torch.Tensor:
...
def main():
    ...
    for i, (attr_idx, (prefix, _part_names)) in enumerate(groundable.items()):
        ...
        t_c = build_ensemble_query(prefix, value, encoder)  # ensemble phrasing -- the ablated axis
    ...
    print("\n=== hybrid CARDS, phrasing=ENSEMBLE (demean=True), vs. real ground truth ===", flush=True)
    ...
    if rho_result is not None:
        print(f"phrasing=ensemble: n={rho_result.n_pairs} rho={rho_result.spearman_rho:+.4f} p={rho_result.spearman_p:.4g} "
    ...
    print(f"phrasing=baseline (v67 baseline, for reference): n={rho_b.n_pairs} rho={rho_b.spearman_rho:+.4f} "
          f"p={rho_b.spearman_p:.4g} | sign={sign_b.agreement_frac:.1%} ({sign_b.n_agree}/{sign_b.n_pairs}) "
          f"binom_p={sign_b.binom_p:.4g}", flush=True)

    with open(RESULTS_DIR / "cards_cub_masking_hybrid_phrasing_scores.csv", "w", newline="") as f:
        writer = csv.writer(f)
        writer.writerow(["attribute_index", "attribute_name", "attribute_prefix", "native_class_idx", "hybrid_raw_score"])
        writer.writerows(raw_rows)
    print(f"\nSaved {len(raw_rows)} rows to results/cards_cub_masking_hybrid_phrasing_scores.csv")
