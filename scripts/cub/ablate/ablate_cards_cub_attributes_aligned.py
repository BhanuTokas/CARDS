"""v44's full K x demean_query x phrasing-ensembling x encoder grid,
re-run with `aligned_retrieval` (cards.retrieval.aligned) in place of
`matched_retrieval` -- prompted directly by "Let's set aligned as
default for now. Run the whole sweep on both aligned and stratified?"
Identical structure to ablate_cards_cub_attributes.py otherwise (same
14 configs: 12 SigLIP x [K in 15/30/50, demean in F/T, phrasing in
baseline/ensemble] + 2 CLIP x [demean in F/T] at k=30/baseline), so the
two files' results are directly comparable -- the only variable that
changed is retrieval strategy.
"""
...
from ablate_cards_cub_attributes import ALT_PREFIX_TEMPLATES, ENCODER_CONFIGS
...
def main():
    ...
    with open(RESULTS_DIR / "cards_cub_attribute_ablation_aligned.csv", "w", newline="") as f:
        writer = csv.writer(f)
    ...
    print(f"\nSaved {len(results)} configs to results/cards_cub_attribute_ablation_aligned.csv")
