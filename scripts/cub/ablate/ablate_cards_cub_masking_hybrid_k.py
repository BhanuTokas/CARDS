"""K ablation for the CUB masking hybrid, prompted directly ("Can we do
an ablation on K?"). demean=True/orthogonalize=False held fixed
(matching the ORIGINAL v67 baseline, same one-factor-at-a-time
discipline as v70/v71/v72) -- only K varies. Tests K=15 and K=30 against
the existing K=50 baseline (v67), matching CARDS' own historical K-sweep
grid (v44/v47: 15/30/50) for continuity. Both new values are CHEAPER
than K=50 (fewer images retrieved per concept), so this loops both in
one run rather than needing separate scripts.
"""
...
def load_k50_baseline() -> dict[tuple[int, int], float]:
    scores = {}
...
def main():
    ...
    all_rows = []  # (K, attribute_index, attribute_name, attribute_prefix, native_class_idx, hybrid_raw_score)
    results = []  # (K, rho_result, sign_result)
    ...
    for K in K_VALUES:
        ...
        for i, (attr_idx, (prefix, _part_names)) in enumerate(groundable.items()):
            ...
            for native_idx in range(mean_delta.shape[0]):
                ...
                all_rows.append((K, attr_idx, attr_name, prefix, native_idx, score))

            if (i + 1) % 20 == 0 or i + 1 == len(groundable):
        ...
        results.append((K, rho_result, sign_result))
    ...
    print(f"\nK=50 (v67 baseline, for reference): n={rho_b.n_pairs} rho={rho_b.spearman_rho:+.4f} "
          f"p={rho_b.spearman_p:.4g} | sign={sign_b.agreement_frac:.1%} ({sign_b.n_agree}/{sign_b.n_pairs}) "
          f"binom_p={sign_b.binom_p:.4g}", flush=True)

    with open(RESULTS_DIR / "cards_cub_masking_hybrid_k_ablation_raw_scores.csv", "w", newline="") as f:
        writer = csv.writer(f)
        writer.writerow(["K", "attribute_index", "attribute_name", "attribute_prefix", "native_class_idx", "hybrid_raw_score"])
        writer.writerows(all_rows)

    with open(RESULTS_DIR / "cards_cub_masking_hybrid_k_ablation.csv", "w", newline="") as f:
        writer = csv.writer(f)
        writer.writerow(["K", "n_pairs", "spearman_rho", "spearman_p", "sign_agreement", "n_agree", "binom_p"])
        for K, rho_result, sign_result in results:
    ...
    print(f"\nSaved results to results/cards_cub_masking_hybrid_k_ablation.csv "
          f"and results/cards_cub_masking_hybrid_k_ablation_raw_scores.csv")
