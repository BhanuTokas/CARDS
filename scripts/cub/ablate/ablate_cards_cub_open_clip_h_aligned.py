"""Re-runs the open_clip_h (ViT-H-14) encoder ablation under
`aligned_retrieval` -- v46's original open_clip_h test used
`matched_retrieval` (the default at the time), never re-checked after
v47 established `aligned` as the new default. Prompted directly ("run
ablation with other encoders"), alongside v46/v47's own precedent that
retrieval strategy interacts with encoder (aligned helped SigLIP
substantially, barely moved CLIP).

3 configs: k=30/demean=False, k=30/demean=True (matching v46's original
CLIP comparison points), plus k=50/demean=True (v47's SigLIP-winning
setting, to check whether it transfers to this encoder too).
"""
...
def run_config(groundable, attribute_names, encoder, spec, pool, native_model, k: int, use_demean: bool, text_center):
...
def main():
    ...
    print("\nLoading open_clip_h (ViT-H-14/laion2b_s32b_b79k) + pool...", flush=True)
    ...
    results = []
    for k, use_demean in [(30, False), (30, True), (50, True)]:
        label = f"open_clip_h_aligned_k{k}_demean{use_demean}"
        scores = run_config(groundable, attribute_names, encoder, spec, pool, native_model, k, use_demean, text_center)
    ...
    with open(RESULTS_DIR / "cards_cub_open_clip_h_aligned_ablation.csv", "w", newline="") as f:
        writer = csv.writer(f)
    ...
    print(f"\nSaved {len(results)} configs to results/cards_cub_open_clip_h_aligned_ablation.csv")
