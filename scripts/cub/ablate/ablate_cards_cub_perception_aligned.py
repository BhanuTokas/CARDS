"""Re-runs the Perception Encoder (PE-Core-B16-224) ablation under
`aligned_retrieval` -- v46's original perception test used
`matched_retrieval`, never re-checked after v47 established `aligned` as
the new default. Same 3 configs as
ablate_cards_cub_open_clip_h_aligned.py: k=30/demean=False,
k=30/demean=True, k=50/demean=True.
"""
...
ENCODER_CFG = {"name": "perception_encoder", "_target_": "cards.encoders.perception_encoder.PerceptionEncoder",
...
def main():
    ...
    print("\nLoading Perception Encoder (PE-Core-B16-224) + pool...", flush=True)
    ...
    for k, use_demean in [(30, False), (30, True), (50, True)]:
        label = f"perception_aligned_k{k}_demean{use_demean}"
    ...
    with open(RESULTS_DIR / "cards_cub_perception_aligned_ablation.csv", "w", newline="") as f:
        writer = csv.writer(f)
    ...
    print(f"\nSaved {len(results)} configs to results/cards_cub_perception_aligned_ablation.csv")
