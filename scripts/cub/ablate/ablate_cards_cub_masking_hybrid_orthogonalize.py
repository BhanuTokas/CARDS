"""orthogonalize ablation for the CUB masking hybrid, prompted directly
("Should we ablate orthogonalization, demeaning and prompt variations?"
-> "Can we check orthogonalize first?"). demean_query=True held fixed
(matching v67's own baseline, and v70's finding that demean barely
matters for the hybrid on CUB) -- only orthogonalize=True is new here.

Structural difference from every other hybrid script: `cards.pipeline.
orthogonalize_queries` needs the FULL set of 87 already-built (and
demeaned) queries at once (a joint Lowdin/Gram-matrix operation), so
this script builds ALL queries up front before looping concepts for
retrieval/localization/masking, rather than building-and-immediately-
using one query at a time the way every other hybrid script does.
"""
...
def main():
    ...
    # Build ALL 87 queries first (demean=True, matching v67), then
    # jointly orthogonalize the full set -- orthogonalize_queries needs
    # every query at once, unlike every other hybrid script's
    # one-at-a-time loop.
    print("Building all 87 concept queries and jointly orthogonalizing...", flush=True)
    raw_queries = {}
    attr_meta = {}  # attr_idx -> (attr_name, prefix)
    for attr_idx, (prefix, _part_names) in groundable.items():
        ...
        raw_queries[attr_name] = t_c
        attr_meta[attr_name] = (attr_idx, prefix)
    queries = orthogonalize_queries(raw_queries)

    hybrid_scores: dict[tuple[int, int], float] = {}
    raw_rows = []

    for i, (attr_name, (attr_idx, prefix)) in enumerate(attr_meta.items()):
        t_c = queries[attr_name]
        ...
        print(f"[{i + 1:>3d}/{len(attr_meta)}] {attr_name:<40s} n_images={len(delta_logits)}/{K}", flush=True)

    print("\n=== hybrid CARDS, orthogonalize=TRUE (demean=True), vs. real ground truth ===", flush=True)
    ...
    if rho_result is not None:
        print(f"orthogonalize=True: n={rho_result.n_pairs} rho={rho_result.spearman_rho:+.4f} p={rho_result.spearman_p:.4g} "
              f"| sign={sign_result.agreement_frac:.1%} ({sign_result.n_agree}/{sign_result.n_pairs}) "
              f"binom_p={sign_result.binom_p:.4g}", flush=True)

    baseline = load_demean_true_baseline()
    rho_b = score_method_agreement(faithfulness_records, baseline, min_samples_per_pair=3)
    sign_b = score_sign_agreement(faithfulness_records, baseline, min_samples_per_pair=3)
    print(f"orthogonalize=False (v67 baseline, for reference): n={rho_b.n_pairs} rho={rho_b.spearman_rho:+.4f} "
          f"p={rho_b.spearman_p:.4g} | sign={sign_b.agreement_frac:.1%} ({sign_b.n_agree}/{sign_b.n_pairs}) "
          f"binom_p={sign_b.binom_p:.4g}", flush=True)

    with open(RESULTS_DIR / "cards_cub_masking_hybrid_orthogonalize_scores.csv", "w", newline="") as f:
        writer = csv.writer(f)
        writer.writerow(["attribute_index", "attribute_name", "attribute_prefix", "native_class_idx", "hybrid_raw_score"])
        writer.writerows(raw_rows)
    print(f"\nSaved {len(raw_rows)} rows to results/cards_cub_masking_hybrid_orthogonalize_scores.csv")
