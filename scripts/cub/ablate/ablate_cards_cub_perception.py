"""Fourth encoder ablation point, 87-attribute bank: Meta's Perception
Encoder (PE-Core-B16-224 -- configs/encoder/perception_encoder.yaml),
mirroring the CLIP/open_clip_h comparisons (k=30, baseline phrasing,
demean on/off). Standalone script so it can run as an independent
background process alongside ablate_cards_cub_open_clip_h.py.
"""
...
def main():
    ...
    for use_demean in [False, True]:
        label = f"perception_k30_demean{use_demean}_baseline"
    ...
    with open(RESULTS_DIR / "cards_cub_perception_ablation.csv", "w", newline="") as f:
        writer = csv.writer(f)
    ...
    print(f"\nSaved {len(results)} configs to results/cards_cub_perception_ablation.csv")
