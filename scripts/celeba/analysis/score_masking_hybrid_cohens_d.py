"""Applies Cohen's d standardization to the masking hybrid's per-concept
score, prompted directly ("Can we apply the Cohen'd normalization and
then measure results?") -- `raw_score` is currently just mean(delta_
scores) across a concept's present set; this computes the paired-sample
effect size d_z = mean(delta_scores) / std(delta_scores, ddof=1)
instead, dividing out how NOISY/variable the per-image masking effect
is, not just its average magnitude. Flagged as a legitimate future
addition in `cards.attribution.masking_mode`'s own docstring (Step 7
normalization intentionally skipped for masking_hybrid, "a future
paired-effect-size normalization over delta_scores, Cohen's-d-style").

Run across the SAME 3 pools as the cross-dataset investigation
(official-val, LFW, UTKFace), Attractive task, v113's winning config --
re-scores rho/sign using d_z instead of raw_score for each, to see
whether standardizing by per-image variance changes the cross-dataset
degradation picture (e.g. if LFW/UTKFace's raw scores are noisier, not
just smaller/differently-signed, d_z could reveal a cleaner signal
raw_score's magnitude-only view missed -- or could make no difference,
...
POOLS = {
    "official_val": None,  # special-cased below (official-val CelebA path list, not a plain image dir)
...
def main():
    ...
    out_path = RESULTS_DIR / "cards_celeba_masking_hybrid_cohens_d_comparison.csv"
    ...
    raw_path = RESULTS_DIR / "cards_celeba_masking_hybrid_cohens_d_raw.csv"
    ...
    print("\n=== summary: raw_mean vs cohens_d, per pool ===")
