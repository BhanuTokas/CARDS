...
Also reports Pearson r alongside Spearman rho (direct request), and adds
three more comparison arms beyond the original 4: TCAV's `mean_magnitude`
(vs. `mean_sign_count`, direct request -- magnitude has no natural
positive/negative split, so `skip_sign=True` for it), PCBM's whole-image
CAV bank variant, and PCBM's "CLIP concepts" variant (Yuksekgonul et al.
2023, no CAV fitting -- concept vectors are the encoder's own text
embeddings) on both a standard CLIP RN50 and a SigLIP backbone (direct
request).
"""
...
def load_pcbm_clip_concepts_scores(csv_name: str) -> dict[str, dict[tuple[int, int], float]]:
    ...
    SigLIP text embeddings of the concept names) -- two backbones, reading
    `results/<csv_name>` directly (written by
    `train_pcbm_clip_concepts_celeba_full.py`)."""
    by_task: dict[str, dict[tuple[int, int], float]] = {t: {} for t in TARGET_CLASSES}
    with open(RESULTS_DIR / csv_name, newline="") as f:
        for row in csv.DictReader(f):
...
def _stats_for(records, scores, method_threshold: float, skip_sign: bool):
    """-> dict[task_name] = (rho_r, sign_r), rho_r possibly None if too few
    pairs, sign_r always None when skip_sign (a non-negative score, e.g.
    TCAV magnitude, has no meaningful positive/negative split to test)."""
    out = {}
    for task_name in TARGET_CLASSES:
        rho_r = score_method_agreement(records[task_name], scores[task_name])
        sign_r = None if skip_sign or rho_r is None else score_sign_agreement(
            records[task_name], scores[task_name], method_threshold=method_threshold)
        out[task_name] = (rho_r, sign_r)
    return out


def report(method: str, pool: str, old_records, new_records, scores_by_task, rows: list,
           method_threshold: float = 0.0, skip_sign: bool = False):
    """Prints BOTH ground-truth versions side by side (Spearman + Pearson +
    optional sign agreement) AND appends one row per task to `rows` for the
    CSV write -- computed together per call instead of two separate
    top-level report() blocks, so old/new never have to be re-matched
    afterward."""
    print(f"\n=== {method}, {pool} ===", flush=True)
    old_stats = _stats_for(old_records, scores_by_task, method_threshold, skip_sign)
    new_stats = _stats_for(new_records, scores_by_task, method_threshold, skip_sign)
    for task_name in TARGET_CLASSES:
        ...
        old_line = (f"  [{task_name}] orig:      n={rho_old.n_pairs} rho={rho_old.spearman_rho:+.4f} (p={rho_old.spearman_p:.4g})  "
                    f"pearson_r={rho_old.pearson_r:+.4f} (p={rho_old.pearson_p:.4g})")
        new_line = (f"  [{task_name}] corrected: n={rho_new.n_pairs} rho={rho_new.spearman_rho:+.4f} (p={rho_new.spearman_p:.4g})  "
                    f"pearson_r={rho_new.pearson_r:+.4f} (p={rho_new.pearson_p:.4g})")
        if sign_old is not None:
            old_line += f"  sign={sign_old.agreement_frac:.1%} ({sign_old.n_agree}/{sign_old.n_pairs}, p={sign_old.binom_p:.4g})"
        if sign_new is not None:
            new_line += f"  sign={sign_new.agreement_frac:.1%} ({sign_new.n_agree}/{sign_new.n_pairs}, p={sign_new.binom_p:.4g})"
        print(old_line, flush=True)
        ...
        rows.append({
            ...
            "pearson_r_orig": rho_old.pearson_r, "pearson_p_orig": rho_old.pearson_p,
            "sign_frac_orig": sign_old.agreement_frac if sign_old else "",
            "sign_p_orig": sign_old.binom_p if sign_old else "",
            "n_corrected": rho_new.n_pairs, "rho_corrected": rho_new.spearman_rho, "rho_p_corrected": rho_new.spearman_p,
            "pearson_r_corrected": rho_new.pearson_r, "pearson_p_corrected": rho_new.pearson_p,
            "sign_frac_corrected": sign_new.agreement_frac if sign_new else "",
            "sign_p_corrected": sign_new.binom_p if sign_new else "",
        })
...
def main():
    ...
    report("TCAV (mean_magnitude)", "-", old_records, new_records, tcav_magnitude_scores, rows, skip_sign=True)
    ...
    with open(out_path, "w", newline="") as f:
        writer = csv.DictWriter(f, fieldnames=[
            "method", "pool", "task", "n_orig", "rho_orig", "rho_p_orig", "pearson_r_orig", "pearson_p_orig",
            "sign_frac_orig", "sign_p_orig",
            "n_corrected", "rho_corrected", "rho_p_corrected", "pearson_r_corrected", "pearson_p_corrected",
            "sign_frac_corrected", "sign_p_corrected",
        ])
        writer.writeheader()
