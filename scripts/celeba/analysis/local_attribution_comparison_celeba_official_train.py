"""Official-train counterpart of `local_attribution_comparison_celeba_
corrected_gt.py` -- SAME local (per-image, per-concept) attribution
comparison (baseline/hybrid/tcav vs. real masking ground truth,
top-10-by-|magnitude| grids), prompted directly ("Can we run this
analysis with the new official celebA trained Attractive and Male
predicting model?"). Only the classifier and its consequences change:

  - **Backbone**: `celeba_official_train_attractive_male` (predicts
    Attractive+Male) instead of `celeba_attractive_young` (Attractive+
    Young). `TASK_POSITIVE_LOGIT_INDEX = {"Attractive": 1, "Male": 3}`,
    matching `run_tcav_celeba_official_train.py`'s own already-
    established convention for this classifier's 4-way logit layout.
  - **Ground truth**: `celeba_official_train_faithfulness_non_
    overlapping.csv` (the CLEAN version -- 5,817 CelebAMask-HQ images
    falling in official CelebA's val/test, genuinely never seen by
    this classifier's training, confirmed disjoint from official
    train) instead of `celeba_full_faithfulness_attribute_conditioned.
    csv`. Already built with the SAME corrected methodology (concept-
    attribute-conditioned, both label directions pooled) this script
    needs -- see `run_celeba_official_train_faithfulness.py`'s own
    docstring.
  - **TCAV exemplar/random/control pools AND the hybrid z-score-cutoff
    candidate images**: sourced from official CelebA TRAIN (`list_
    attr_celeba.txt`/`list_eval_partition.txt` partition==0) instead
    of CelebAMask-HQ's own `train_hq` -- matching what THIS classifier
    actually trained on, the same substitution `run_tcav_celeba_
    official_train.py` already made for its own exemplar pools.
  - **CARDS baseline's own reference/comparison pool** (`absent_
    images`, used only for the excluded-from-grids `baseline` rho row)
    is left UNCHANGED -- still CelebAMask-HQ's own val split via
    `load_celeba`, since it's a generic comparison pool, not tied to
    classifier identity.

Everything else (query building/orthogonalization, hybrid's localize+
threshold+mask+angle-selection mechanism, TCAV's directional-derivative
mechanism, the top-10-by-|magnitude| grid with sign shown) is IDENTICAL
-- see the original script's own docstring for the full shared design.
"""
...
TOP10_DIR = RESULTS_DIR / "local_attribution_top10_official_train"
GT_CSV = RESULTS_DIR / "celeba_official_train_faithfulness_non_overlapping.csv"
...
def load_ground_truth_rows() -> list[dict]:
    ...
    with open(GT_CSV, newline="") as f:
        for row in csv.DictReader(f):
            rows.append({
                "image": row["image"], "concept_name": row["concept_name"],
                "target_task": row["target_task"], "delta_p": float(row["delta_p"]),
            })
    return rows
...
def main():
    ...
    print(f"Loaded {len(gt_rows)} raw (image, concept, task) ground-truth rows "
          f"(official-train classifier, non_overlapping GT).", flush=True)
    ...
    print(f"CARDS-baseline reference pool (CelebAMask-HQ val, unchanged): {len(pool.paths)} images", flush=True)
    ...
    print("\n=== per-concept setup (N_c, z-score cutoff, TCAV CAVs) -- one-time cost, exemplars from OFFICIAL TRAIN ===", flush=True)
    ...
    train_paths_all = [CELEBA_ROOT / "img_align_celeba" / f for f in train_files]
    print(f"{len(train_files)} official-train images (TCAV/hybrid fitting resource).", flush=True)

    rng_py = random.Random(SEED)
    all_train_paths = list(train_paths_all)
    ...
    tcav = TCAV(model=native_model_cpu, layers=[HOOK_LAYER], save_path=str(RESULTS_DIR / "local_attribution_official_train_tcav_cav_cache"))
    ...
    for c_i, concept_name in enumerate(GROUNDABLE_CONCEPTS):
        ...
        for p in pos_train_paths[:50]:
            ...
            sim_maps.append(localize_concept(encoder, img, t_c_hybrid.to(DEVICE), (img.height, img.width)))
    ...
    out_path = RESULTS_DIR / "local_attribution_celeba_official_train_pairs.csv"
    ...
    print(f"\nSaved {len(all_out_rows)} scored rows to {out_path}", flush=True)

    print("\n=== local rho (n = all scored rows, official-train classifier) ===", flush=True)
