"""Phase 7 scale-up of the CelebA plan: TCAV against the native
celeba_attractive_young model, for ALL 26 groundable concepts against
BOTH target tasks -- extends run_tcav_celeba_pilot.py's own 8-concept
pilot run (v71) the same way. Logic is IDENTICAL to the pilot script
(whole-image real-attribute-label concept definition, target=1/target=3
straight into the native 4-way head, N_VAL_SAMPLES-image subsample per
task) -- see that script's own docstring for the full design rationale.
"""
...
def main():
    ...
    tcav = TCAV(model=native_model, layers=[spec.hook_layer], save_path=str(RESULTS_DIR / "tcav_celeba_full_cav_cache"))
    ...
    with open(RESULTS_DIR / "tcav_celeba_full_scores.csv", "w", newline="") as f:
        writer = csv.DictWriter(f, fieldnames=list(results[0].keys()))
        writer.writeheader()
        writer.writerows(results)

    print(f"\n{len(results)} total (concept, task) TCAV pairs saved to results/tcav_celeba_full_scores.csv")
