"""Whole-image PCBM variant -- fits PCBM's own linear classifier as a
SURROGATE of the native celeba_attractive_young model's own predictions,
using fit_celeba_full_whole_image_cavs.py's whole-image concept bank
instead of the region-crop one. Logic, including both bugs the region-
crop script found and fixed (the float64-vs-float32 dtype mismatch, and
the binary-task single-decision-function-row shape gotcha), is otherwise
IDENTICAL to train_pcbm_surrogate_celeba_full.py -- see that script's own
docstring for the full design rationale and the bug writeups. Only the
concept bank path and output naming differ.
"""
...
CONCEPT_BANK_PATH = "trained_concepts_new/celeba_full_whole_image/celeba_attractive_young/celeba_full_whole_image_celeba_attractive_young_0.1_100.pkl"
OUT_DIR = Path("trained_models_new/celeba_full_whole_image/celeba_attractive_young")
SEED = 42
...
def main():
    ...
    print("Loading whole-image concept bank...", flush=True)
    ...
    for task_name in TARGET_CLASSES:
        ...
        model_path = OUT_DIR / f"pcbm_celeba_full_whole_image__celeba_attractive_young__{task_name.lower()}__surrogate__seed_{SEED}__linear.ckpt"
