"""TCAV extension to the Male task, mirroring `run_tcav_celeba_full.py`'s
own full-scale design exactly (whole-image real-attribute-label concept
definition, N_VAL_SAMPLES-image subsample, same random/control-pool
construction) but hooking the NEW `celeba_attractive_young_male`
backbone (target logit index 5, the 3rd 2-way softmax block) instead of
the original 2-task model -- prompted directly ("Can we extend to add
Male to our analysis?" -> "Full 3-method comparison").

Random/control concept pools are rebuilt fresh from the SAME
train_hq split (not reused from the original run) since they need to
be encoded through the Male backbone's own (different-weights)
embedding space, not the original checkpoint's.
"""
...
def main():
    ...
    tcav = TCAV(model=native_model, layers=[spec.hook_layer], save_path=str(RESULTS_DIR / "tcav_celeba_male_cav_cache"))
    ...
    val_indices = positive_val[:N_VAL_SAMPLES]
    val_paths = [image_paths_by_idx[i] for i in val_indices]
    ...
    for concept_name in GROUNDABLE_CONCEPTS:
        ...
        scores = tcav.interpret(inputs=inputs, experimental_sets=experimental_sets, target=MALE_POSITIVE_LOGIT_INDEX)
        target_sign_counts = [
        ...
        control_scores = tcav.interpret(inputs=inputs, experimental_sets=control_sets, target=MALE_POSITIVE_LOGIT_INDEX)
        control_sign_counts = [
        ...
        results.append({
            "concept_name": concept_name, "target_task": TASK_NAME, "n_val_images": len(val_paths),
    ...
    with open(RESULTS_DIR / "tcav_celeba_male_scores.csv", "w", newline="") as f:
        writer = csv.DictWriter(f, fieldnames=list(results[0].keys()))
        writer.writeheader()
        writer.writerows(results)

    print(f"\n{len(results)} total concept TCAV rows saved to results/tcav_celeba_male_scores.csv")
