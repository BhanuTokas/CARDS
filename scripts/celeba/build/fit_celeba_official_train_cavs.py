"""CAV refit for the NEW official-train classifier, prompted directly
("Do we also have results of PCBM and TCAV on this version of ground
truth?" -- they didn't exist yet, building them now). Reuses the SAME
region-crop bank (`Datasets/celeba_full_concepts/`, unchanged) but
embeds through `celeba_official_train_attractive_male`'s own embedding
space instead -- required, not optional, same reasoning as v117's own
Male-backbone CAV refit: this is a genuinely separately-trained model,
different weights throughout the whole network, not the same space.
"""
...
OUT_DIR = Path("trained_concepts_new/celeba_full/celeba_official_train_attractive_male")
...
def main():
    ...
    for C in C_VALUES:
        out_path = OUT_DIR / f"celeba_full_celeba_official_train_attractive_male_{C}_{2 * N_SAMPLES}.pkl"
