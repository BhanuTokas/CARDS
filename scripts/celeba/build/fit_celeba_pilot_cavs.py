"""Phase 6 (PCBM half) of the CelebA plan: fits CAVs for the 8-concept
pilot from build_celeba_pilot_concept_bank.py's real region-crop bank,
against celeba_attractive_young's own embedding space -- the CelebA-track
analogue of scripts/cub/build/fit_cub_part_cavs.py, structurally
identical (same post_hoc_cbm.learn_concept_bank call, same output pickle
convention: {concept_name: (cav_vector, train_acc, test_acc, intercept,
margin_info)} per C, a drop-in ConceptBank/PosthocLinearCBM input).
"""
...
from cards.data.celeba_attributes import PILOT_CONCEPTS
from cards.models.backbones import BACKBONES

CONCEPT_ROOT = Path(r"C:\Users\btokas\Projects\Datasets\celeba_pilot_concepts")
OUT_DIR = Path("trained_concepts_new/celeba_pilot/celeba_attractive_young")
DEVICE = "cuda" if torch.cuda.is_available() else "cpu"
N_SAMPLES = 50  # matches post_hoc_cbm's own CUB default -- needs 2*50=100 pos/neg per concept; every pilot concept has 150 of each
C_VALUES = [0.01, 0.1]
BATCH_SIZE = 25
SEED = 42
...
class FlattenedFeatureExtractor(nn.Module):
    """Matches post_hoc_cbm's own ResNetBottom exactly (backbone stack,
    native head dropped, explicit flatten)."""
...
def main():
    ...
    for C in C_VALUES:
        out_path = OUT_DIR / f"celeba_pilot_celeba_attractive_young_{C}_{2 * N_SAMPLES}.pkl"
