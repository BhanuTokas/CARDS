"""Trains a NEW black-box classifier on standard CelebA's own OFFICIAL
train partition (162,770 images, `list_eval_partition.txt` part==0),
NOT CelebAMask-HQ's 25,500-image curated train split every other
classifier in this track used -- prompted directly ("Can we train a
classifier on the CelebA official train set and see if it holds the
same pattern?"), to test whether the whole method-comparison finding
(masking hybrid beats TCAV/PCBM) survives a classifier trained on the
full, much larger, standard-resolution official distribution instead
of the smaller HQ-curated one. Also drops Young per direct instruction
("we can drop Young from our analysis") -- 2 tasks now (Attractive,
Male), not 3.

**Leakage check done directly before writing this script, not
assumed**: CelebAMask-HQ's 30,000 images are NOT disjoint from official
train -- 24,183/30,000 (80.6%) fall inside it. Training on all of
official train would contaminate any ground truth built from those
images (they carry the only real segmentation masks in this whole
track). The clean held-out set for ground-truth generation this time is
the 5,817 CelebAMask-HQ images that fall in official VAL (2,993) or
TEST (2,824) instead -- genuinely never seen by this classifier's
gradient updates. (A companion ground-truth script consumes this same
5,817-image set; not built here.)

Images are standard low-resolution `img_align_celeba/` captures (178x218
native) -- plain resize-to-224 preprocessing, NOT the special HQ-
degradation trick `_celeba_lowres_preprocess` used (that trick
simulates low-res capture from an HQ source; these are genuinely
captured at low-res already, nothing to simulate).

Same architecture/training convention as the original classifier
(`train_attractive_young_classifier.py`): ResNet18, ImageNet-pretrained
init, 2 independent 2-way softmax blocks ([0:2]=Attractive,
[2:4]=Male), same optimizer/LR split, same early-stopping-on-val-loss
gate (>70% val acc on both tasks).
"""
...
def main():
    ...
    ckpt_path = CKPT_DIR / "resnet18_official_train_attractive_male.pt"
    ...
    with open(CKPT_DIR / "run_info_official_train.pkl", "wb") as f:
        pickle.dump(run_info, f)
    print(f"Saved run_info to {CKPT_DIR / 'run_info_official_train.pkl'}", flush=True)
