"""ConvNeXt-Tiny counterpart of `train_official_celeba_classifier.py` --
SAME data/head convention/training recipe as the ViT counterpart (see
`train_official_celeba_classifier_vit.py`'s own docstring for the full
shared design). Only the backbone architecture and head-swap mechanics
differ.

`model.classifier` is torchvision's own `Sequential(LayerNorm2d, Flatten,
Linear(768,1000))` -- unlike ViT's single-module `heads`, only the FINAL
element (`classifier[2]`) is replaced, keeping the pretrained LayerNorm2d
+ Flatten in place (they're not learnable-in-a-meaningful-sense-for-a-
"head" the way the final Linear is, and swapping the whole Sequential
would drop the normalization the rest of the pretrained network expects
before its classification layer). Backbone/head param split keys off
"classifier.2." (the new layer's own parameter-name prefix), NOT
"classifier." broadly -- classifier.0 (LayerNorm2d) stays at backbone_lr
along with the rest of the pretrained network.
"""
...
def build_model() -> nn.Module:
    ...
    model.classifier[2] = nn.Linear(in_features, 4)  # [0:2]=Attractive [2:4]=Male
    return model
...
def main():
    ...
    backbone_params = [p for name, p in model.named_parameters() if not name.startswith("classifier.2.")]
    ...
    ckpt_path = CKPT_DIR / "convnext_tiny_official_train_attractive_male.pt"
    ...
    with open(CKPT_DIR / "run_info_official_train_convnext.pkl", "wb") as f:
        pickle.dump(run_info, f)
    print(f"Saved run_info to {CKPT_DIR / 'run_info_official_train_convnext.pkl'}", flush=True)
