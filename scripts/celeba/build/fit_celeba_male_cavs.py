"""CAV refit for the Male task, prompted directly ("Can we extend to add
Male to our analysis?" -> "Full 3-method comparison"). Reuses `fit_
celeba_full_cavs.py`'s SAME region-crop bank (`Datasets/
celeba_full_concepts/`, unchanged -- the crops themselves don't depend
on which classifier the CAVs get fit against) but embeds them through
the NEW `celeba_attractive_young_male` backbone instead of the original
2-task model.

Refitting (not reusing the original CAVs) is required, not optional:
the Male-extended checkpoint is a genuinely SEPARATE training run
(same architecture, different final weights throughout the whole
network including the backbone, since the extra Male loss term changes
gradients everywhere) -- its own 512-dim embedding space is not the
same space the original CAVs were fit in, confirmed by construction
(different `state_dict`, not a superset/subset relationship).
"""
...
OUT_DIR = Path("trained_concepts_new/celeba_full/celeba_attractive_young_male")
...
def main():
    ...
    for C in C_VALUES:
        out_path = OUT_DIR / f"celeba_full_celeba_attractive_young_male_{C}_{2 * N_SAMPLES}.pkl"
