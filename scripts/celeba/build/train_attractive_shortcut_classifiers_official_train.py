"""Official-train version of the shortcut-learning validation experiment
(`train_attractive_shortcut_classifiers.py`), prompted directly ("Can
we repeat the experiment but train on the official train set with
shortcuts instead please?"). SAME shortcut mechanism, SAME architecture/
hyperparameters, SAME single-target (2-way head) simplification -- the
only change is the image source: standard CelebA's OFFICIAL train
partition (162,770 images, `img_align_celeba`, plain resize-224
preprocessing) instead of CelebAMask-HQ's 25,500-image curated split,
matching `train_official_celeba_classifier.py`'s own data source.

Scope, corrected twice directly -- ("Wait. I needed you to run the 10%
intervals": an initial draft defaulted to just the 4 canonical rates
0%/33%/67%/100%, the ORIGINAL headline experiment's own scope, before
this first correction to the finer sweep) then ("We do not need 0.33
and 0.67": dropping those 2 non-decile rates, keeping only the clean
10%-step grid): 11 rates, 0/10/20/.../100%. This is ~6.4x more images
per checkpoint than CelebAMask-HQ, so even 11 rates here is
substantially more expensive than the original 4-rate run.

**HPC-portable, prompted directly ("I needed Male and Attractive. Can
you write the scripts and I can run it on HPC in the interest of
compute?")** -- `CELEBA_ROOT`/`CKPT_DIR`/`RESULTS_DIR` are all env-var
overridable (falling back to this machine's own local Windows paths),
matching the established convention in `scripts/ftw/run/run_
conceptmask_ftw_dual_window.py`. On HPC: set `CELEBA_ROOT` to wherever
standard CelebA (`img_align_celeba/`, `list_attr_celeba.txt`,
`list_eval_partition.txt`) lives there; `CKPT_DIR`/`RESULTS_DIR`
default to relative paths that work as long as the job runs from the
repo root, same as every other script in this project.

Companion script: `train_male_shortcut_classifiers_official_train.py`
(SAME design, target=Male instead of Attractive, prompted in the same
request -- "I needed Male and Attractive").

Two DISTINCT shortcuts, one per class: a 10x10px solid-color block,
fixed top-left corner (4px margin) -- magenta (255,0,255) for
target=1, cyan (0,255,255) for target=0. Injected AFTER resize/flip/
color-jitter, directly on the final 224x224 frame, same correctness
reasoning as the original script (flip/jitter must not relocate or
distort the cue).

Val accuracy reported BOTH on the same-rate-injected official val split
(early stopping) AND on a clean (rate=0) version of the SAME val images
(shortcut-reliance diagnostic), matching the original script's design.
"""
...
def train_one_rate(rate: float, train_files, train_labels, val_files, val_labels) -> dict:
    ...
    ckpt_path = CKPT_DIR / f"resnet18_official_train_attractive_shortcut_{rate_pct}.pt"
    info_path = CKPT_DIR / f"run_info_official_train_attractive_shortcut_{rate_pct}.pkl"
