"""4-method variant of plot_predicted_vs_actual_celeba_full.py, adding
the gradient-based pixel-attribution baseline (run_gradient_attribution_
celeba_full.py, prompted directly: "Can we use the Gradient based pixel
attribution as baseline?") as a 4th column -- the first method in this
entire investigation (CUB included) to significantly beat chance at
true scale. 2-row x 4-col grid; logic otherwise identical to the
3-method version.
"""
...
OUT_PATH = RESULTS_DIR / "predicted_vs_actual_celeba_full_with_gradient.png"
...
def load_gradient_scores() -> dict[str, dict[tuple[int, int], float]]:
    by_task: dict[str, dict[tuple[int, int], float]] = {t: {} for t in TARGET_CLASSES}
...
def main():
    ...
    method_cols: list[tuple[str, dict[str, dict[tuple[int, int], float]], float]] = [
        ...
        ("Gradient (Integrated Gradients,\nn_steps=20, mask contrast)", gradient_by_task, 0.0),
    ]
    ...
    fig.suptitle(
        f"Predicted vs. actual concept importance -- CelebA full 26-concept bank "
        f"(+ gradient baseline), n_ground_truth_records={n_total_records}\n"
