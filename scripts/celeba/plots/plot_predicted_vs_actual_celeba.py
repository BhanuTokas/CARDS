...
scatter plots for the CelebA 8-concept pilot -- structural template
scripts/cub/plots/plot_predicted_vs_actual_cub.py, same per-panel
diagnostic (aggregated via the SAME `_aggregate_faithfulness_pairs`
helper `score_method_agreement`/`score_sign_agreement` use internally,
quadrant lines at x=threshold/y=0, green=sign-agreeing/red=sign-
disagreeing points, rho+sign-agreement in each panel's subtitle).

Unlike CUB (one shared class space, so one grid of method panels), this
track has 2 independent target tasks -- so the grid here is 2 rows
(Attractive, Young) x 3 columns (CARDS, TCAV, PCBM), each panel's
ground-truth records pre-filtered to that row's own `target_task`
(Phase 4's own design note: predicted_class is constant=1 across both
tasks here, so target_task is what disambiguates them, unlike CUB's
species-id-valued predicted_class which needed no such filtering).
"""
...
OUT_PATH = RESULTS_DIR / "predicted_vs_actual_celeba_pilot.png"

CONCEPT_TO_IDX = {name: i for i, name in enumerate(PILOT_CONCEPTS)}
...
def load_records_by_task() -> dict[str, list[FaithfulnessResult]]:
    ...
    with open(RESULTS_DIR / "celeba_pilot_faithfulness.csv", newline="") as f:
...
def load_cards_scores() -> dict[str, dict[tuple[int, int], float]]:
    ...
    with open(RESULTS_DIR / "cards_celeba_pilot_scores.csv", newline="") as f:
        for row in csv.DictReader(f):
...
def load_tcav_scores() -> dict[str, dict[tuple[int, int], float]]:
    by_task: dict[str, dict[tuple[int, int], float]] = {t: {} for t in TARGET_CLASSES}
    with open(RESULTS_DIR / "tcav_celeba_pilot_scores.csv", newline="") as f:
        for row in csv.DictReader(f):
...
def load_pcbm_scores() -> dict[str, dict[tuple[int, int], float]]:
    ...
    for task_name in TARGET_CLASSES:
        ckpt_path = (
            Path("trained_models_new/celeba_pilot/celeba_attractive_young")
            / f"pcbm_celeba_pilot__celeba_attractive_young__{task_name.lower()}__surrogate__seed_42__linear.ckpt"
        )
        if not ckpt_path.exists():
            by_task[task_name] = {}
            continue
        posthoc_layer = torch.load(ckpt_path, weights_only=False)
        # sklearn's SGDClassifier.coef_ for a BINARY task is a single
        # decision-function row (classes_ sorted [0, 1], positive value
        # -> predicted class 1), not one row per class the way a >2-class
        # PCBM (e.g. CUB's 200-way) would have -- set_weights reassigns
        # classifier.weight to that (1, n_concepts) shape regardless of
        # the n_classes=2 the layer was built with (confirmed directly
        # against a real saved checkpoint, not assumed). Row 0 IS the
        # task-positive-class importance per concept, so no [1, :] index
        # into a second row exists here.
...
def plot_panel(ax, name: str, records: list[FaithfulnessResult], scores: dict[tuple[int, int], float],
    ...
    ax.scatter(x, y, c=colors, s=28, alpha=0.75, edgecolors="none")
...
def main():
    ...
    fig.suptitle(
        f"Predicted vs. actual concept importance -- CelebA 8-concept pilot, n_ground_truth_records={n_total_records}\n"
