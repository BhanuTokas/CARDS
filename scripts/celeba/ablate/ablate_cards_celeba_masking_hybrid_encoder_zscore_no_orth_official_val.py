...
rerun on the official (non-HQ) CelebA val pool -- prompted directly
("What validation split are you using?" -> "The validation split needs
to be the CelebA's official split."), correcting the same
HQ-val-by-default mistake as v101/v102 before it. The HQ-val version
(`ablate_cards_celeba_masking_hybrid_encoder_zscore_no_orth.py`) is kept
as a separate real data point, not deleted -- same "both pools, both
kept" precedent as v101/v102.

Pool: the SAME 16,874-image "clean" official-val pool used throughout
this thread. Ground truth: `results/celeba_full_faithfulness.csv` (the
only one available -- see the official-val threshold sweep's own
docstring for why this is still valid: it's aggregated per (concept,
task), independent of which images any given method scores on).

Otherwise identical to the HQ-val version: K=50, demean=True,
orthogonalize=False, alpha in {1.0, 1.5}, all 4 encoders, localization
cached once per concept and reused across both alpha values.
"""
...
def main():
    ...
    for encoder_name, encoder_cfg in ENCODERS_TO_RUN.items():
        print(f"\n=== {encoder_name} (K={K}, demean=True, orthogonalize=False, official-val) ===", flush=True)
    ...
    with open(RESULTS_DIR / "cards_celeba_masking_hybrid_encoder_zscore_no_orth_official_val_ablation.csv", "w", newline="") as f:
        writer = csv.writer(f)
    ...
    print(f"\nSaved {len(results)} (encoder_alpha, task) rows to results/cards_celeba_masking_hybrid_encoder_zscore_no_orth_official_val_ablation.csv")

    with open(RESULTS_DIR / "cards_celeba_masking_hybrid_encoder_zscore_no_orth_official_val_ablation_raw_scores.csv", "w", newline="") as f:
        writer = csv.writer(f)
        writer.writerow(["encoder", "alpha", "concept_name", "target_task", "hybrid_raw_score"])
        writer.writerows(raw_score_rows)
    print(f"Saved {len(raw_score_rows)} raw score rows to results/cards_celeba_masking_hybrid_encoder_zscore_no_orth_official_val_ablation_raw_scores.csv")
