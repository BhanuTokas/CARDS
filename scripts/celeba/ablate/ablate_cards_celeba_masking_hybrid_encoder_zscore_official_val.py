"""Encoder ablation for the z-score threshold's own best config (alpha=1.0,
orthogonalize=True), rerun on the official (non-HQ) CelebA val pool --
prompted directly ("Can you also run an encoder ablation as well
please!" -> "And a K ablation as well please!" -> "But, the point was to
run on the official val pool without the overlaps right?" -- the first
attempt at this ablation mistakenly defaulted to CelebA-HQ's own val
split, matching v83/v93's own precedent, instead of continuing the
official-val thread that was the actual point of this whole follow-up;
see ablate_cards_celeba_masking_hybrid_encoder_zscore.py for that
HQ-val version, kept as a separate real data point, not deleted).

Pool: the SAME 16,874-image "clean" official-val pool v98/v99 already
built for SigLIP -- CLIP/open_clip_h/Perception each need their OWN
fresh encode pass here (a new cache key per encoder), unlike the HQ-val
version of this ablation which could reuse each encoder's pool from
v83/v93. SigLIP's own official-val alpha=1.0 reference comes from the
official-val z-score sweep itself (ablate_cards_celeba_masking_hybrid_
threshold_zscore_official_val.py's own alpha=1.0 row), NOT v96's HQ-val
number -- a genuinely different pool needs a genuinely different
reference point.

Otherwise identical to the HQ-val version: K=50, demean=True,
orthogonalize=True, z-score alpha=1.0, the 7-strategy fill family, the NEW
integrated cards.attribution.localization library code.
"""
...
# SigLIP's own official-val alpha=1.0 numbers (from the official-val z-score
# sweep's own k=1.0 row -- NOT v96's HQ-val reference).
SIGLIP_OFFICIAL_VAL_REFERENCE = {
...
def main():
    ...
    for encoder_name, encoder_cfg in ENCODERS_TO_RUN.items():
        print(f"\n=== {encoder_name} (K={K}, demean=True, orthogonalize=True, z-score alpha={ALPHA}, official-val) ===", flush=True)
    ...
    print("\n=== siglip (K=50, demean=True, orthogonalize=True, z-score alpha=1.0, official-val) -- "
          "reference from the official-val z-score sweep, not recomputed ===", flush=True)
    for task_name in TARGET_CLASSES:
        r = SIGLIP_OFFICIAL_VAL_REFERENCE[task_name]
        print(f"  [{task_name}] rho={r['rho']:+.4f} p={r['rho_p']:.4g} | sign={r['sign']:.1%} p={r['sign_p']:.4g}", flush=True)

    with open(RESULTS_DIR / "cards_celeba_masking_hybrid_encoder_zscore_official_val_ablation.csv", "w", newline="") as f:
        writer = csv.writer(f)
    ...
    print(f"\nSaved {len(results)} (encoder, task) rows to results/cards_celeba_masking_hybrid_encoder_zscore_official_val_ablation.csv")

    with open(RESULTS_DIR / "cards_celeba_masking_hybrid_encoder_zscore_official_val_ablation_raw_scores.csv", "w", newline="") as f:
        writer = csv.writer(f)
        writer.writerow(["encoder", "concept_name", "target_task", "hybrid_raw_score"])
        writer.writerows(raw_score_rows)
    print(f"Saved {len(raw_score_rows)} raw score rows to results/cards_celeba_masking_hybrid_encoder_zscore_official_val_ablation_raw_scores.csv")
