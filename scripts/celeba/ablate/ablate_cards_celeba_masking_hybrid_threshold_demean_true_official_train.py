"""Fine-grid (0.25 steps) alpha ablation for the masking hybrid on the
official-train CelebA classifier (ResNet18), at demean_query=**True**
-- prompted directly ("Also, rerun the alpha ablation experiment
please", in the same demean=True context as `ablate_cards_celeba_
masking_hybrid_k_demean_true_official_train.py`).

Unlike the demean=False alpha ablation (built in two stages: a coarse
0.5-step grid, then a fine 0.25-step extension reusing the coarse
grid's own 5 values), this one goes straight to the full 10-point
0.25-step grid in one script -- alpha=1.0 is the only REUSED cell
(exactly the (siglip, orth=True, demean=True) cell from the joint
encoder ablation); the other 9 alpha values (0.25, 0.5, 0.75, 1.25,
1.5, 1.75, 2.0, 2.25, 2.5) are all computed fresh here.

Fixed otherwise: SigLIP, orthogonalize=True, K=50 -- same as the
demean=False alpha ablations, only the modality-correction axis
differs.

Hand-rolled (concept-outer, alpha-inner), same design as the
demean=False threshold ablation scripts -- localization is cached once
per concept and reused across all 9 new alpha thresholds.
"""
...
ALPHA_VALUES_NEW = [0.25, 0.5, 0.75, 1.25, 1.5, 1.75, 2.0, 2.25, 2.5]
ALPHA_REUSED = 1.0
...
def load_reused_alpha1_scores() -> dict[str, dict[tuple[int, int], float]]:
    ...
    with open(REUSED_RAW_CSV, newline="") as f:
        for row in csv.DictReader(f):
            if row["encoder"] != "siglip" or row["orthogonalize"] != "True" or row["demean_query"] != "True":
                continue
...
def main():
    ...
    queries = orthogonalize_queries(raw_queries)  # orth=True, demean=True
    ...
    out_path = RESULTS_DIR / "cards_celeba_masking_hybrid_threshold_demean_true_official_train_ablation.csv"
    ...
    raw_path = RESULTS_DIR / "cards_celeba_masking_hybrid_threshold_demean_true_official_train_ablation_raw_scores.csv"
    ...
    cutoff_path = RESULTS_DIR / "cards_celeba_masking_hybrid_threshold_demean_true_official_train_ablation_cutoffs.csv"
