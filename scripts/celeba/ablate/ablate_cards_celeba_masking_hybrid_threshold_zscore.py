...
z-score cutoff (mean + k*std, pooled across a concept's own present-set
...
then derive that concept's own cutoff = mean + k*std for each k in
K_VALUES -- a genuinely two-pass-per-concept design (the cutoff can't be
...
5 k values x 26 concepts x 50 images each.
"""
...
def main():
    ...
    # unchanged across every k below (only the threshold varies).
    raw_queries = {
    ...
    # hybrid_scores_by_k[k][task_name][(concept_idx, 1)] = raw_score
    hybrid_scores_by_k: dict[float, dict[str, dict[tuple[int, int], float]]] = {
        k: {t: {} for t in TARGET_CLASSES} for k in K_VALUES
    }
    all_rows = []  # (k, concept_name, target_task, hybrid_raw_score)
    cutoff_rows = []  # (k, concept_name, cutoff, mean_area_pct)
    ...
    for c_i, concept_name in enumerate(GROUNDABLE_CONCEPTS):
        ...
        # this once (not once per k) avoids K_VALUES redundant forward
        ...
        # Pass 2: one cutoff per k, then mask/fill/score using the cached maps.
        for k in K_VALUES:
            cutoff = concept_zscore_cutoff(sim_maps, k)
            areas = []
            ...
            cutoff_rows.append((k, concept_name, cutoff, mean_area_pct))
            for task_name in TARGET_CLASSES:
                score = float(np.mean(delta_logits[task_name])) if delta_logits[task_name] else 0.0
                hybrid_scores_by_k[k][task_name][(concept_idx, 1)] = score
                all_rows.append((k, concept_name, task_name, score))

            print(f"    k={k:<4.1f} cutoff={cutoff:+.4f} mean_area={mean_area_pct:>5.1f}% "
    ...
    for k in K_VALUES:
        print(f"\n=== k={k} ===", flush=True)
        for task_name in TARGET_CLASSES:
            rho_result = score_method_agreement(records_by_task[task_name], hybrid_scores_by_k[k][task_name])
            sign_result = score_sign_agreement(records_by_task[task_name], hybrid_scores_by_k[k][task_name])
    ...
    with open(RESULTS_DIR / "cards_celeba_masking_hybrid_threshold_zscore_orthogonalize_ablation_cutoffs.csv", "w", newline="") as f:
        writer = csv.writer(f)
        writer.writerow(["k", "concept_name", "cutoff", "mean_area_pct"])
        writer.writerows(cutoff_rows)

    print(f"\nSaved {len(results)} (k, task) rows to results/cards_celeba_masking_hybrid_threshold_zscore_orthogonalize_ablation.csv")
