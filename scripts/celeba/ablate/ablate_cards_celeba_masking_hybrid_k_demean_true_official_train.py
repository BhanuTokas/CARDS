...
classifier (ResNet18), at demean_query=**True** (the OLDER default the
rest of the official-train pipeline still uses) -- prompted directly
("Run experiments on demean=True for now, also test of K =
{15,30,50,75,100}"), a deliberate departure from `ablate_cards_celeba_
masking_hybrid_k_official_train.py`'s own demean_query=False setting
(the new best-known config from the joint encoder ablation).

Fixed otherwise: SigLIP, orthogonalize=True, z-score alpha=1.0 -- same
as the demean=False K ablation, only the modality-correction axis and
the K grid differ (K now also covers 75 and 100, not just 15/30/50).

K=50's own cell is REUSED, not recomputed -- exactly the (siglip,
orth=True, demean=True) cell already computed by the joint encoder
ablation. Only K in {15, 30, 75, 100} are computed fresh here.
...
def main():
    ...
    out_path = RESULTS_DIR / "cards_celeba_masking_hybrid_k_demean_true_official_train_ablation.csv"
    ...
    raw_path = RESULTS_DIR / "cards_celeba_masking_hybrid_k_demean_true_official_train_ablation_raw_scores.csv"
    ...
    print("\n=== summary (non_overlapping GT), sorted by K ===")
    non_overlap = [r for r in all_rows if r[2] == "non_overlapping"]
    for K in sorted({r[0] for r in non_overlap}):
        for task_name in TASKS:
            r = next((r for r in non_overlap if r[0] == K and r[1] == task_name), None)
            if r:
                print(f"  K={r[0]:<4d} task={r[1]:<10s} rho={r[4]:+.4f} sign={r[6]:.1%} [{r[9]}]")
