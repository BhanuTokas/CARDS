"""K ablation for the CelebA masking hybrid, prompted directly ("Can we
do an ablation on K?"). demean=True/orthogonalize=False held fixed
(matching the ORIGINAL v82 baseline, same one-factor-at-a-time
discipline as v86/v87/v88) -- only K varies. Tests K=15 and K=30 against
the existing K=50 baseline (v82), matching CARDS' own historical K-sweep
grid. Both new values are CHEAPER than K=50, so loops both in one run.
"""
...
def load_k50_baseline() -> dict[str, dict[tuple[int, int], float]]:
...
def main():
    ...
    baseline_by_task = load_k50_baseline()
    print("\n=== K=50 (v82 baseline, for reference) ===", flush=True)
    for task_name in TARGET_CLASSES:
        rho_b = score_method_agreement(records_by_task[task_name], baseline_by_task[task_name])
        sign_b = score_sign_agreement(records_by_task[task_name], baseline_by_task[task_name])
        print(f"  [{task_name}] n={rho_b.n_pairs} rho={rho_b.spearman_rho:+.4f} p={rho_b.spearman_p:.4g} "
              f"| sign={sign_b.agreement_frac:.1%} ({sign_b.n_agree}/{sign_b.n_pairs}) binom_p={sign_b.binom_p:.4g}", flush=True)

    with open(RESULTS_DIR / "cards_celeba_masking_hybrid_k_ablation.csv", "w", newline="") as f:
        writer = csv.writer(f)
    ...
    with open(RESULTS_DIR / "cards_celeba_masking_hybrid_k_ablation_raw_scores.csv", "w", newline="") as f:
        writer = csv.writer(f)
        writer.writerow(["K", "concept_name", "target_task", "hybrid_raw_score"])
        writer.writerows(all_rows)

    print(f"\nSaved results to results/cards_celeba_masking_hybrid_k_ablation.csv "
          f"and results/cards_celeba_masking_hybrid_k_ablation_raw_scores.csv")
