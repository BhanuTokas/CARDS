"""Encoder ablation for the z-score threshold WITHOUT orthogonalize,
prompted directly ("Do we want to test the other encoders without
orthogonalization?" -> "Should we not test alpha=1.5?"). v93 found
orthogonalize=True's own benefit is SigLIP-specific and actively HURTS
Perception (at top_pct=15); v101 found the same pattern at z-score
alpha=1.0 (SigLIP the only encoder with significant rho, on either
pool). Neither of those isolates WHETHER it's orthogonalize dragging
the other 3 encoders down, or the z-score threshold itself -- this
tests alpha WITHOUT orthogonalize across all 4 encoders to find out.
v83's own precedent (the ORIGINAL non-orthogonalized best-of-N
mechanism) found all 4 encoders converge cleanly, so there's real
reason to expect orthogonalize is the actual culprit, not confirmed
until tested.

Two alpha values, not just v96's own HQ-val-tuned optimum (1.0):
alpha=1.5 was the actual BEST config on the official-val pool (v100,
rho=+0.4885) and a close second on HQ-val (+0.5986 vs alpha=1.0's
+0.6150) -- testing only alpha=1.0 here would risk re-introducing the
exact HQ-val-tuning bias this whole encoder/orthogonalize thread is
trying to get out from under.

No orthogonalize means no joint 26-query step -- each concept's
(demeaned) query is built independently, same as v82's own original
per-concept loop. Localization is cached ONCE per concept (concept-outer
design, same as the z-score ablation's own pattern) and reused across
both alpha values, since orthogonalize=False and alpha don't change
retrieval/localization, only the threshold.

HQ-val pool (matches v83/v93/v101's own precedent for encoder
ablations). No existing reference values -- z-score without orthogonalize
has not been tested for any encoder before this.
"""
...
def main():
    ...
    results = []
    raw_score_rows = []  # (encoder, alpha, concept_name, target_task, hybrid_raw_score)

    for encoder_name, encoder_cfg in ENCODERS_TO_RUN.items():
        print(f"\n=== {encoder_name} (K={K}, demean=True, orthogonalize=False) ===", flush=True)
        ...
        queries = {c: demean_query(build_concept_query(CONCEPT_QUERY_TEXT[c], encoder), text_center) for c in GROUNDABLE_CONCEPTS}
        ...
        for concept_idx, concept_name in enumerate(GROUNDABLE_CONCEPTS):
            ...
            for alpha in ALPHA_VALUES:
                ...
                for task_name in TARGET_CLASSES:
                    ...
                    hybrid_scores_by_alpha[alpha][task_name][(CONCEPT_TO_IDX[concept_name], 1)] = score
                    raw_score_rows.append((encoder_name, alpha, concept_name, task_name, score))
        ...
        for alpha in ALPHA_VALUES:
            ...
            for task_name in TARGET_CLASSES:
                evaluate(f"{encoder_name}_alpha{alpha}", task_name, records_by_task[task_name],
                          hybrid_scores_by_alpha[alpha][task_name], results)

    with open(RESULTS_DIR / "cards_celeba_masking_hybrid_encoder_zscore_no_orth_ablation.csv", "w", newline="") as f:
        writer = csv.writer(f)
        writer.writerow(["encoder_alpha", "target_task", "n_pairs", "spearman_rho", "spearman_p",
    ...
    print(f"\nSaved {len(results)} (encoder_alpha, task) rows to results/cards_celeba_masking_hybrid_encoder_zscore_no_orth_ablation.csv")

    with open(RESULTS_DIR / "cards_celeba_masking_hybrid_encoder_zscore_no_orth_ablation_raw_scores.csv", "w", newline="") as f:
        writer = csv.writer(f)
        writer.writerow(["encoder", "alpha", "concept_name", "target_task", "hybrid_raw_score"])
        writer.writerows(raw_score_rows)
    print(f"Saved {len(raw_score_rows)} raw score rows to results/cards_celeba_masking_hybrid_encoder_zscore_no_orth_ablation_raw_scores.csv")
