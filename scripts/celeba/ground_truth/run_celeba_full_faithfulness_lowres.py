"""Regenerates the masking-based faithfulness ground truth against the
LOW-RESOLUTION-trained classifier (celeba_attractive_young_lowres),
resolution-mismatch follow-up to run_celeba_full_faithfulness.py
(notes/celeba_correlation_investigation.md).

Identical methodology, SAME 4,500-image CelebA-HQ val split, SAME real
per-pixel masks, SAME 100-per-(concept,task) sampling -- only the
classifier being explained changes. Since cards.models.backbones.
_celeba_lowres_preprocess degrades every image to standard CelebA's own
178x218 native resolution before its final 224x224 resize, this
regenerates ground truth for "what does the low-res-trained classifier's
own output do when the real region is masked out of a (now artificially
degraded) CelebA-HQ image" -- the correct comparison for isolating the
train/eval resolution-mismatch hypothesis, not a mismatched one.

Saves to results/celeba_full_faithfulness_lowres.csv -- a SEPARATE file
from the original, which is left untouched (still used by every
existing config's own reported numbers).
"""
...
class CelebaTaskAdapter:
    """Same shape as run_celeba_full_faithfulness.py's own adapter, just
    pointed at the low-res-trained checkpoint."""
...
def main():
    ...
    with open(RESULTS_DIR / "celeba_full_faithfulness_lowres.csv", "w", newline="") as f:
    ...
    print(f"\n{len(results)} total faithfulness records saved to results/celeba_full_faithfulness_lowres.csv", flush=True)
